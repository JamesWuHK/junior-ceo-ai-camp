...
export function issueTeacherToken(teacher: TeacherPrincipal) {
  ...
  const signature = sign(`${tokenPrefix}.${payload}`);
  return {
    token: `${tokenPrefix}.${payload}.${signature}`,
...
export function verifyTeacherToken(token: string): TeacherPrincipal | null {
  ...
  if (prefix !== tokenPrefix || !payload || !signature) return null;
