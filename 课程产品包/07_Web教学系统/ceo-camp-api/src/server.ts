  ...
  return rows<Record<string, any> & { payload?: string }>(
    `SELECT ts.*, s.nickname AS student_name, t.name AS team_name
      ...
      WHERE ts.camp_id = ?
        ...
        AND ts.task_type IN ('problem_card', 'market_scout', 'user_voice', 'ai_validation', 'product_definition', 'prompt_card', 'feature_scope', 'tech_route', 'iteration_plan', 'value_card', 'final_showcase')
...
function projectJourneyItems() {
  return rows<Record<string, any> & { payload?: string }>(
    `SELECT ts.*, s.nickname AS student_name, t.name AS team_name
      ...
      WHERE ts.camp_id = ?
        AND ts.status = 'ON_WALL'
        AND ts.task_type IN ('problem_card', 'market_scout', 'user_voice', 'ai_validation', 'product_definition', 'prompt_card', 'feature_scope', 'tech_route', 'product_feedback', 'iteration_plan', 'value_card', 'mentor_comment')
