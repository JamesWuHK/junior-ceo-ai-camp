#!/usr/bin/env python3
"""V3: Embedded live demo chat widget on slides — actually type prompts and see AI responses"""
...
<script>let c=0;const s=document.querySelectorAll(".slide"),t=s.length;function show(i){{s.forEach(x=>x.classList.remove("active"));s[i].classList.add("active");document.getElementById("pageNum").textContent=(i+1)+" / "+t}}function next(){{c=(c+1)%t;show(c)}}function prev(){{c=(c-1+t)%t;show(c)}}document.addEventListener("keydown",e=>{{if(e.key==="ArrowRight"||e.key==="ArrowDown")next();if(e.key==="ArrowLeft"||e.key==="ArrowUp")prev()}})
{chatJs}
</script></body></html>'''

CHAT_JS = '''
// Embedded chat demo — handles AI-like responses for demo prompts
function initChat(containerId, presets){
  ...
  function getResponse(text){
    const t = text.trim();
    // Match presets
    for(const [pattern, response] of Object.entries(presets)){
      if(t.includes(pattern)) return response;
    }
    // Fallback
    return '正在生成回复...\\n\\n（这是一个模拟演示，在实际课堂中这里会是 AI 的真实回答）';
  }
...
def mod(fn, title, slides, chat_js=''):
    t = slides.count('<div class="slide')
    html = WRAPPER.replace('{title}', title).replace('{slides}', slides).replace('{total}', str(t)).replace('{chatJs}', chat_js)
...
<div class="slide"><h1>他们有什么<br>共同点？</h1><div class="hint think" style="margin-top:16px">思考 · 1 分钟</div><div class="hint-text">两人各发现了什么？做了什么？赚到了什么？</div></div>
...
<div class="slide"><h2>算一笔账</h2><div class="formula" style="margin-top:20px;font-size:clamp(20px,3vw,32px)">100 个用户 × 每人 <span>¥__</span> / 月<br>= 一个月赚 <span>¥__</span></div><div class="hint do" style="margin-top:24px">实操</div><div class="hint-text">每组站起来，大声喊出你们的数字！</div><p style="margin-top:16px;font-size:17px">定价不是数学题，是共情题</p></div>
'''
...
print("\n✅ 7 modules with embedded live chat demos.")
