...
function buildWeeklyPriorityReport({ generatedAt, config, evidenceSnapshot, geoSnapshot }) {
  ...
  return [
    ...
    '',
    `1. Refresh the working CSV: \`npm run seo:weekly-init\` writes ${WEEKLY_PRIVATE_CHECKLIST_CSV_FILE}; use \`--force\` only when you intentionally want to replace local private evidence notes.`,
    `2. Complete the ${evidenceSnapshot.urlRows.length} P0 URL_INDEX rows first. A row counts only after \`indexed\`, \`evidenceDate\`, \`source\`, and a short note identify Baidu Search Resource Platform data or a reproducible browser \`site:\` result.`,
    `3. Complete the ${priorityRankRows.length} P1 KEYWORD_RANK rows next. For manual SERP checks, record rank or explicit no-rank evidence plus date, city/network, device, browser state, and exact query.`,
    `4. Complete the ${priorityGeoRows.length} P1 GEO_ANSWER rows with one AI answer check per cluster. Record engine, date, exact prompt, whether the project is mentioned, whether the target page/context is used, and whether positioning is accurate.`,
